...
void C_labWork::WrlabWork() {
    ...
    m_titlePage.s_labWorkNum = {C_labWork::TypeThecking(m_titlePage.s_labWorkNum)};
    ...
    m_titlePage.s_operSystem = {C_labWork::GetOsName()};

    // устанавливаем локаль(кодировка)
    m_titlePage.s_loc = LocToWstring(std::setlocale(LC_ALL, nullptr));
...
void C_labWork::WrTask() {
    ...
    m_task.s_taskNum = TypeThecking(m_task.s_taskNum, static_cast<ushort>(ProgrEnum::Task_2),
...
wstr C_labWork::LocToWstring(const char* input) {
    std::wstring_convert<std::codecvt_utf8<wchar_t>> converter;
    return converter.from_bytes(input);
}

// функция выбора задания
ProgrEnum C_labWork::TaskSelec(const ushort f_choice) {
...
const bool C_labWork::StopLab() {
    ...
    wchar_t choice = {C_labWork::TypeThecking(choice)}; // проверка на корректность ввода;
