...
// указатель на функцию замены символов
void (*pReplaceAll)(wstr& f_str, const wstr f_from, const wstr f_to){ReplaceAll};

// указатель на функцию преобразования локали в std::wstring
wstr (*pLocToWstr)(const char* input){LocToWstr};

// указатель на функцию определения операционной системы
const wstr (*pGetOsName)(){GetOsName};

// указатель на функцию выбора задания
const ProgrEnum (*pTaskSelec)(const ushort& f_choice){TaskSelec};

// указатель на функцию проверка на корректность ввода номера задания
const ushort& (*pTypeThecking)(ushort& f_a, const ushort& f_min, const ushort& f_max){TypeThecking};

// указатель на функцию проверка на коректность ввода выхода из программы
const wchar_t& (*pTypeThecking1)(wchar_t& f_c){TypeThecking};
    ...
    if (!f_objLabWork.empty()) {
        ...
        if (MemoError(m_titlePage.s_objLabWork)) {
            ...
            *m_titlePage.s_objLabWork = f_objLabWork;
            ...
            // Заменяем ';' на '\n'
            ReplaceAll(*m_titlePage.s_objLabWork, L";", L"\n");
        }
...
// конструктор с параметрами о лабораторной работе по умочанию
C_labWork::C_labWork(const ushort& f_labWorkNum, const wstr& f_topLabWork, const wstr& f_objLabWork) {
...
void C_labWork::WrOperSystem() {
    ...
    if (MemoError(m_titlePage.s_objLabWork)) {
        ...
        *m_titlePage.s_operSystem = {pGetOsName()};
    } else DelObj(m_titlePage.s_operSystem); // иначе удаляем объект
}

// устанавливаем локаль(кодировка) лабораторной работы
void C_labWork::WrLoc() {
    wstr loc{pLocToWstr(std::setlocale(LC_ALL, nullptr))};
...
// устанавливаем номер задания
void C_labWork::WrTaskNum() {
        ...
        m_task.s_taskNum = pTypeThecking(m_task.s_taskNum, static_cast<ushort>(ProgrEnum::Task_2),
                                              static_cast<ushort>(ProgrEnum::Task_Max));
}

// читаем номер задания
const ushort& C_labWork::RdTaskNum() const {return m_task.s_taskNum;}
...
wstr LocToWstr(const char* f_input) {
    if (!f_input) {
        return L"";
    }
    wstr result{};
...
const ProgrEnum TaskSelec(const ushort& f_choice) {
...
const ushort& TypeThecking(ushort& f_a, const ushort& f_min, const ushort& f_max) {
    while (true) {
        ...
        if (std::wcin >> f_a) {
            ...
            if (f_a >= f_min && f_a <= f_max) {
                ...
                errmess::Exeption(L"Номер вне допустимого диапазона [" +
...
const wchar_t& TypeThecking(wchar_t& f_c) {
    while (true) {

        // Пытаемся считать символ
        if (std::wcin >> f_c) {

            // если введенный символ число
            if (isdigit(f_c)) {
...
void PrintLab(const C_labWork& f_lab, ushort f_fieldWidth = 25, wchar_t f_separator = L'=') {

    // проверяем указатели на nullptr
    if (!f_lab.m_titlePage.s_labWorkNum || !f_lab.m_titlePage.s_topLabWork ||
        !f_lab.m_titlePage.s_objLabWork || !f_lab.m_titlePage.s_operSystem ||
        !f_lab.m_titlePage.s_loc || f_lab.m_devLop.s_devLop.empty() || f_lab.m_devLop.s_group.empty())
    {
...
void PrintTask(const C_labWork& f_lab, ushort f_fieldWidth, wchar_t f_separator) {
    if (!f_lab.m_task.s_taskNum || f_lab.m_task.s_taskText.empty()) {
...
const bool Stop(const wstr& str) {
    bool  stop{false};

    // запрос пользователю на выбор действия
    errmess::Info(str);
    wchar_t choice{TypeThecking(choice)}; // проверка на корректность ввода;
